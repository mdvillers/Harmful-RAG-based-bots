...
Usage examples:
uv run prompt_injection_scripts/inject_prompts_in_dataset.py \
...
Features:
- Supports prompt sources in 'autodan' JSON (map of strategies with Example lists),
  a plain JSON list of strings, or a newline-delimited text file.
- Randomly selects prompts and appends them to each entry's `answers` list with a random date.
- Logging reports number of available prompts and total injections performed.
"""
...
def load_prompts(prompts_path: Path, fmt: str) -> List[str]:
	...
	if fmt == "list":
		...
		if not isinstance(data, list):
			raise ValueError("prompts file with format 'list' must be a JSON array of strings")
...
def inject_prompts_into_jsonl(source_path: Path, prompts: List[str], per_entry: int, output_path: Path, fraction: float = 1.0):
	...
	with source_path.open("r", encoding="utf-8") as src, output_path.open("w", encoding="utf-8") as out:
		for line in src:
			line = line.strip()
			if not line:
				continue
			...
			if do_inject:
				for _ in range(per_entry):
					...
					answers.append({"answer_text": wrapped_prompt, "answer_time": random_date_str()})
					injected += 1
...
def main():
	parser = argparse.ArgumentParser(description="Inject prompts into a JSONL QA dataset")
	parser.add_argument("--source-file", required=True, help="Path to source JSONL file")
	parser.add_argument("--prompts-file", required=True, help="Path to prompts file (JSON or text)")
	parser.add_argument("--prompts-format", choices=["autodan", "list", "text"], default="autodan", help="Format of prompts file")
	parser.add_argument("--per-entry", type=int, default=1, help="How many prompts to inject per entry (default 1)")
	parser.add_argument("--fraction", type=float, default=1.0, help="Fraction of entries to inject (0.0-1.0). Default 1.0 = all entries")
	parser.add_argument("--output-file", required=False, help="Output JSONL path (defaults to <source>_with_injections.jsonl)")
	...
	out_path = Path(args.output_file) if args.output_file else source_path.with_name(source_path.stem + "_with_injections.jsonl")

	if args.seed is not None:
		random.seed(args.seed)

	prompts = load_prompts(prompts_path, args.prompts_format)
	logger.info("Loaded %d prompts from %s (format=%s)", len(prompts), prompts_path, args.prompts_format)
	if not prompts:
